...
def validate(rule_name, rule, df):
    ...
    invalid_list = json.dumps([{"value": row[rule_name], "count": row["count"]} for row in invalid_list], indent=2)
...
for dq_section in config:
    if dq_section.startswith("dq_"):
        ...
        if "accuracy" in dq_config["measure"] and target_df is not None:
            ...
            source_df.select("longitude").show()
            print(*groupby_cols)
            ...
            for rule_name, rule in accuracy_rules.items():
                ...
                # accuracy_check = accuracy_check.withColumn(f"{target_col}_accuracy", (col(f"{col_prefix}_{source_col}") - col(f"{col_prefix}_{target_col}")) <= threshold)

                accuracy_check.show()
                # accuracy_check.filter(accuracy_check["latitude_accuracy"] != True).show()
